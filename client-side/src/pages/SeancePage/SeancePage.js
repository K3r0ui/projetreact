...
const SeancePage = () => {
      ...
      const fetchData = async () => {
         ...
         if (data2) {
            ...
            data2.map((seance, index) => {
               let resultFormat = formatStatComp(
                  seance.competences,
                  seance.statistiques
               );
               ...
               dataTable.push({
                  ...
                  joueur:
                     seance.joueur.firstName + ' ' + seance.joueur.lastName,
                  etat: seance.etat,
   ...
   const finish = async (
      ...
      if (seance.status && seance.status === 200) {
         let resultFormat = formatStatComp(
            seance.data.competences,
            seance.data.statistiques
         );
         const newSeance = {
            key: seance.data._id,
            titre: (
               <Link to={'/seance/' + seance.data._id}>
                  {seance.data.titre}
               </Link>
            ),
            ...
            joueur:
               seance.data.joueur.firstName + ' ' + seance.data.joueur.lastName,
            ...
            Action: (
               <>
                  <Button
                     onClick={() => annulerSeance(seance.data._id)}
                     style={{ width: '80%' }}
                     type='primary'>
                     Annuler
                  </Button>
                  <Button style={{ width: '80%' }} type='danger'>
                     Feedback
                  </Button>
               </>
            ),
            ...
            <div className='mt-5 mb-5'>
               <center>
                  <Radio.Group
                     ...
                     <Radio.Button style={{ width: '50%' }} value='all'>
                        toutes les Séances
                     </Radio.Button>

                     <Radio.Button style={{ width: '50%' }} value='now'>
                        {' '}
                        Séances d'aujourd'huit
                     </Radio.Button>
                  </Radio.Group>
                  ...
                  <Select
                     ...
                     {joueurs.map((joueur) => (
                        <Option key={joueur.firstName + ' ' + joueur.lastName}>
                           {joueur.firstName}
                        </Option>
                     ))}
                  </Select>
